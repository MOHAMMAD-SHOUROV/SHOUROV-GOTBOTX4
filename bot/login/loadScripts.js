...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
        ...
        const aliasesData = typeof globalData.get === 'function' ? await globalData.get('setalias', 'data', []) : [];
                                                ...
                                                if (!packageAlready.includes(packageName)) {
                                                        ...
                                                        if (!existsSync(`${process.cwd()}/node_modules/${packageName}`)) {
                                                                const wating = setInterval(() => {
                                                                        // loading.info('SHOUROV-BOT PACKAGE', `${spinner[count % spinner.length]} Installing package ${packageName} for ${text} ${file}`);
                                                                        loading.info('SHOUROV-BOT PACKAGE', `${spinner[count % spinner.length]} Installing package ${colors.yellow(packageName)} for ${text} ${colors.yellow(file)}`);
                                                                        count++;
                                                                }, 80);
                                                                try {
