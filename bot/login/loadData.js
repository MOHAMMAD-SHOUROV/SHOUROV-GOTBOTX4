...
module.exports = async function (api, createLine) {
    ...
    log.info(
        ...
        getText(
            ...
            global.db.allThreadData.filter(t => t.threadID.toString().length > 15).length
        )
    );

    log.info(
        'DATABASE',
        getText('loadData', 'loadUserDataSuccess', global.db.allUserData.length)
    );
    ...
    if (api && global.GoatBot.config.database.autoSyncWhenStart === true) {
        ...
        try {
            ...
            // ✅ SAFE await (Node 20 OK)
            let allThreadInfo = await api.getThreadList(9999999, null, 'INBOX');
            allThreadInfo = allThreadInfo.filter(t => t && t.threadID);
            ...
            for (const threadInfo of allThreadInfo) {
                if (threadInfo.isGroup && !allThreadData.some(t => t.threadID === threadInfo.threadID)) {
                    threadDataWillSet.push(
                        await threadsData.create(threadInfo.threadID, threadInfo)
                    );
                } else {
            ...
            const allThreadDataDontHaveBot = allThreadData.filter(
                t => !allThreadInfo.some(t2 => t.threadID === t2.threadID)
            );
            ...
            log.info(
                'DATABASE',
                getText('loadData', 'refreshThreadDataSuccess', global.db.allThreadData.length)
            );
        }
        catch (err) {
            spin._stop();
            log.error(
                'DATABASE',
                getText('loadData', 'refreshThreadDataError'),
                err
            );
        }
        finally {
